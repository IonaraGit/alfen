...
const Observacao = require('../models/Comentario')
const Comentario = require('../models/Comentario')

router.post('/orcamento/salvar', (req, res) => {
  var id = req.body.id
  var observacao = Array.isArray(req.body.observacao)
    ? req.body.observacao
    : [req.body.observacao]
  var valor = Array.isArray(req.body.valor) ? req.body.valor : [req.body.valor]
  var ambiente = Array.isArray(req.body.ambiente)
    ? req.body.ambiente
    : [req.body.ambiente]
  var marca = Array.isArray(req.body.marca) ? req.body.marca : [req.body.marca]
  var modelo = Array.isArray(req.body.modelo)
    ? req.body.modelo
    : [req.body.modelo]
  var btus = Array.isArray(req.body.btus) ? req.body.btus : [req.body.btus]
  var prestacao = Array.isArray(req.body.prestacao)
    ? req.body.prestacao
    : [req.body.prestacao]
  var quantidade = Array.isArray(req.body.quantidade)
    ? req.body.quantidade
    : [req.body.quantidade]
  var colaboradoreId = Array.isArray(req.body.colaboradoreId)
    ? req.body.colaboradoreId
    : [req.body.colaboradoreId]
  var clienteId = Array.isArray(req.body.clienteId)
    ? req.body.clienteId
    : [req.body.clienteId]
  var empresaId = Array.isArray(req.body.empresaId)
    ? req.body.empresaId
    : [req.body.empresaId]
  var status = 0
...
router.post('/orcamento/agendamento/salvar', (req, res) => {
  ...
  var horario = Array.isArray(req.body.horario)
    ? req.body.horario
    : [req.body.horario]
  var colaboradorId = Array.isArray(req.body.colaborador)
  ...
  Promise.all(agenda)
    .then(ag => {
      ...
      for (let i = 0; i < ag.length; i++) {
        orcamento.push(
          Orcamento.update({ status: 1 }, { where: { id: orcamentoId } })
        )
      }
        ...
        if (orcamento) {
          ...
          if (statusOrcamento !== 2 && statusOrcamento !== 3) {
            ...
            res
              .status(400)
              .redirect(
                `/admin/orcamentos/prosseguir/${id_cliente_a}?error=${mgserro}`
              )
          }
        } else {
...
router.post('/orcamento/atualizar', async (req, res) => {
  ...
  try {
    ...
    let msg =
      'O valor total do orçamento não pode ser menor que o valor já recebido!'

    if (valorAlterado < valor_aberto) {
      // Codificar a mensagem na URL
      const encodedMsg = encodeURIComponent(msg)
      res.redirect(
        `/admin/orcamentos/editar/${id}/${orcamentoId}?error=${encodedMsg}`
      )
    } else {
      ...
      res.redirect(
        `/admin/orcamentos/editar/${id}/${orcamentoId}?mensagem=${testemsg}`
      )
    }
...
router.post('/orcamentos/deletar', async (req, res) => {
  try {
    ...
    let msg =
      'Orçamentos agendados, inicializados e/ou finalizados, não podem ser exluidos!'
    const mgserro = encodeURIComponent(msg)
    ...
    if (idOrcamento !== undefined && !isNaN(idOrcamento)) {
      Agenda.count({
        ...
        if (count > 0) {
          res.redirect(
            `/admin/orcamentos/decisao2/${idCliente}?error=${mgserro}`
          )
        } else {
          Orcamento.destroy({
            where: {
            ...
            Modelo.findAll().then(modelos => {
              Ambiente.findAll().then(ambientes => {
                Endereco.findAll().then(enderecos => {
                  const orcamentosSelecionados = orcamentos.filter(orcamento =>
                    orcamentoIds.includes(orcamento.id.toString())
                  )

                  if (orcamentosSelecionados.length > 0) {
                    const primeiroClienteId =
                      orcamentosSelecionados[0].clienteId
                    const clienteDoOrcamento = clientes.find(
                      cliente => cliente.id == primeiroClienteId
                    )

                    if (clienteDoOrcamento) {
                      const enderecoDoCliente = enderecos.find(
                        endereco => endereco.clienteId == clienteDoOrcamento.id
                      )

                      console.log(
                        chalk.red.bold(
                          'Cliente para o e-mail é: ' + clienteDoOrcamento.email
                        )
                      )
                      ...
                      orcamentosSelecionados.forEach((orcamento, index) => {
                        const prestacaoDoOrcamento = prestacoes.find(
                          prestacao => prestacao.id == orcamento.prestacoId
                        )

                        const btuDoOrcamento = btus.find(
                          btu => btu.id == orcamento.btuId
                        )

                        const marcaDoOrcamento = marcas.find(
                          marca => marca.id == orcamento.marcaId
                        )

                        const modeloDoOrcamento = modelos.find(
                          modelo => modelo.id == orcamento.modeloId
                        )

                        const ambienteDoOrcamento = ambientes.find(
                          ambiente => ambiente.id == orcamento.ambienteId
                        )
                        ...
                        <style>
                          ...
                          <table border="1" class="orcamento-table">
                            ...
                            <tr>
                              ...
                              <td>${
                                prestacaoDoOrcamento
                                  ? prestacaoDoOrcamento.descricao
                                  : 'N/A'
                              }</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                              <th>Valor cobrado</th>
                              <td>R$ ${orcamento.valor.toLocaleString('pt-BR', {
                            ...
                            <tr>
                              ...
                              <td>R$ ${orcamento.valor_recebido.toLocaleString(
                                'pt-BR',
                                {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2
                            ...
                            <tr>
                              ...
                              <td>R$ ${orcamento.valor_aberto.toLocaleString(
                                'pt-BR',
                                {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2
                        ...
                        if (
                          (index + 1) % 2 === 0 &&
                          index < orcamentosSelecionados.length - 1
                        ) {
                          conteudoHTML += `
                      ...
                      <table border="1" class="orcamento-table">
                        <tr>
                          <th>Total cobrado</th>
                          <td style='color: red; font-weight: 600;'>R$ ${somaValoresCobrados.toLocaleString(
                            'pt-BR',
                            {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2
                        ...
                        <tr>
                          ...
                          <td style='color: green; font-weight: 600;'>R$ ${somaValoresPagos.toLocaleString(
                            'pt-BR',
                            {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2
                        ...
                        <tr>
                          ...
                          <td style='color: blue; font-weight: 600;'>R$ ${somaValoresAbertos.toLocaleString(
                            'pt-BR',
                            {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2
                      ...
                      pdf
                        .create(
                          testandobr + headerHTML + dadosCliente + conteudoHTML,
                          pdfOptions
                        )
                        .toFile('orcamento.pdf', (err, res) => {
...
//AREA COLABORADOR
router.post('/orcamento/iniciar/:id', (req, res) => {
  ...
  if (id_orcamento != undefined) {
    ...
    res.redirect('/acesso/colaborador#nav-em-andamento');
  } else {
    res.send('Procure o suporte!')
  }
})

router.post('/orcamento/agendamento/comentar/:id', (req, res) => {
  const id_agendamento = req.params.id
  var descricao = req.body.descricao
  var empresa = req.body.empresa
  var colaborador = req.body.colaborador

  if (id_agendamento != undefined) {
    Comentario.create({
      descricao: descricao,
      empresaId: empresa,
      agendaId: id_agendamento,
      colaboradoreId: colaborador
    })
      .then(() => {
        res.redirect('/acesso/colaborador#nav-em-andamento');
...
router.post('/orcamento/finalizar/:id', (req, res) => {
  const id_orcamento = req.params.id
